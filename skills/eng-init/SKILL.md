...
- `CONTEXT.md` — project identity, bounded contexts, invariants, forbidden logic, and open terminology questions. It **links to** `openspec/glossary.md` for domain-term definitions and never defines a term itself.
- `openspec/glossary.md` — the single home of domain-term definitions across this repo's skills (`grill-me` docs mode, `architecture-design`, `improve-codebase-architecture`, `review` all read and write it). Format: `grill-me`'s `references/GLOSSARY-FORMAT.md` (`## Language`; multi-context repos add `## Context Map` in the same file). Created lazily — only once the first term is confirmed, never as an empty placeholder.
...
- Domain terms → `openspec/glossary.md`; bounded contexts, invariants, forbidden logic, and open terminology conflicts → `CONTEXT.md`.
...
4. **Project memory plan** — `AGENTS.md` sections and `CONTEXT.md` sections to create or repair, the AGENTS.md write target (direct, or `instructions/agents/shared.md` + regenerate), whether `openspec/glossary.md` enters the write set (only when at least one term was confirmed), and the `CLAUDE.md` `@AGENTS.md` import line when Claude Code was named at Q1.5.
...
10. **Non-goals** — files and behavior explicitly out of scope, including any `CLAUDE.md` content beyond the `@AGENTS.md` import line unless the user separately asks.
...
2. `CONTEXT.md` from captured bounded contexts, invariants, forbidden logic, and open terminology questions, linking to `openspec/glossary.md`; the confirmed domain terms themselves go into `openspec/glossary.md` (GLOSSARY-FORMAT), created only when there is at least one term to write.
2a. `CLAUDE.md` containing the single line `@AGENTS.md` when Claude Code was named at Q1.5 and no `CLAUDE.md` exists; when one exists without the import, add that one line (spec-previewed) and nothing else.
...
- `.claude/settings.json` is written only when Claude Code was named at Q1.5 — it is enforcement config (hooks). The only `CLAUDE.md` content eng-init ever writes is the `@AGENTS.md` import line.
...
- If `CLAUDE.md` exists in a project, the only default edit is adding the `@AGENTS.md` import line when it is missing and Claude Code is in use; everything else in it is out of scope unless the user explicitly asks.
- If `openspec/glossary.md` is missing but domain terms appear in code, create a starter glossary there and record uncertain terms under `CONTEXT.md` `## Open Terminology Questions`. If `CONTEXT.md` still carries a `## Domain Language` table from an older eng-init version, propose moving the definitions into `openspec/glossary.md` and leaving a link (retired-section rule).
...
- Do not use project `CLAUDE.md` as the project memory file; it carries the `@AGENTS.md` import and nothing else of eng-init's.
...
| File | When | What |
|---|---|---|
| `references/agent-readiness-criteria.md` | Stage 0/1 and audit-only | Readiness checklist mapped to `AGENTS.md`, `CONTEXT.md`, `openspec/glossary.md`, `CLAUDE.md` bridge, commands, tests, guardrails, plus control-plane layers and AGENTS.md constraint dimensions |
...
| `references/context-md-template.md` | Stage 3/4 | `CONTEXT.md` template: identity, bounded contexts, invariants; domain terms link out to `openspec/glossary.md` |
...
2. **One term, one home.** `openspec/glossary.md` defines domain terms; `CONTEXT.md` owns bounded contexts and invariants and links to it. One term has one meaning; bounded contexts stop multi-agent context bleed.
