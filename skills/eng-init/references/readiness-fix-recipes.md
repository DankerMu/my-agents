...
- **Fixability:** A — skill-owned.
- **Good fixes:** `CONTEXT.md` with project identity, bounded contexts, invariants, forbidden logic, and open terminology questions, linking to `openspec/glossary.md` where the confirmed terms live (GLOSSARY-FORMAT; created only when there is a term to write).
...
- **Allowed files:** `CONTEXT.md` and `openspec/glossary.md`, unless linking from `AGENTS.md` is already part of the write set.
...
- **Fixability:** A — skill-owned.
- **Good fixes:** A `CLAUDE.md` whose first non-blank line is `@AGENTS.md`; when a hand-written `CLAUDE.md` already exists, the import line added at the top and nothing else touched.
- **Bad fixes:** Copying AGENTS.md content into CLAUDE.md (two sources that drift), a symlink on Windows, wrapping the import in backticks or a code fence (Claude Code then treats it as literal text), editing any other line of a hand-written CLAUDE.md.
- **Required scan:** Q1.5 answer or `.claude/` presence (is Claude Code in use?), existing `CLAUDE.md`, whether `AGENTS.md` is generated by `project-instruction-bootstrap` (then the import already comes from its `claude.md` fragment — coordinate, do not duplicate).
- **Allowed files:** `CLAUDE.md` only.
- **Validator:** Read-back: the line `@AGENTS.md` appears outside code spans/fences.
- **Rescore evidence:** `claude_md_bridge=1/1` with the line cited; `null` with the reason when Claude Code is not in use.
