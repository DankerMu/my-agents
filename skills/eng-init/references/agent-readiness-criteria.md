...
| `guardrail_self_test` | 3 | yes | `scripts/test-guardrails.sh` exists and a CI/cron job runs it; the self-test asserts both directions — guards accept the clean tree and reject one staged violation each (a rejection-only self-test is blind to always-failing guards); fixtures that start processes are reaped by the self-test itself (per-fixture pid record, TERM then KILL) and the run ends with a residue assertion that fails on any leftover — audit by snapshotting `ps -A -ww -o pid= -o args=` before and after one run: a new process whose command line names the temp dir is a leak | Enforcement Index |
