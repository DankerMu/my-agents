#!/usr/bin/env python3
"""Run eng-init unit tests in a uv-managed pytest environment.

The suite lives in scripts/tests (not tests/): scripts/selfcheck.sh is one of
the things under test and its fixture tests clone the whole skill.
"""
...
def default_tests_path() -> Path:
    return Path(__file__).resolve().parents[1] / "scripts" / "tests"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run eng-init unit tests with an ephemeral pytest dependency."
    )
    parser.add_argument(
        "--tests-path",
