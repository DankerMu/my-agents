...
name: grill-me
description: >
  对一个已有的 plan、design 或任何待定决策/想法做对抗式压测（不限软件）：把计划展开成决策树、按前沿逐轮追问，直到与用户达成共同理解。
  每轮问完当前前沿的全部问题，每个问题编号并给出你的推荐答案；事实能从环境（codebase、文档、数据）查到的就去查，决策必须交给用户拍板。
...
1. **按前沿逐轮提问**：前沿 = 前置决策都已定、现在就能问而不必猜测未听到答案的那些决策。一轮问完整个前沿（每题编号），等用户答完再算下一轮；答案依赖本轮尚未回答问题的，留到后面的轮次，不在本轮猜。
2. **每个问题都附上你的推荐答案 + 一句理由**，让用户在"确认/否决/修正"之间选择，而不是从零作答。
3. **事实自己查，决策交给人**：凡是能从环境——codebase、设计文档、数据或其它已有产物——查到的**事实**，自己去查，不要拿来占用用户；但**决策**是用户的——每个决策都摆到用户面前、等用户答复，绝不代答。本 skill 被嵌进其它工作流（如 `stage-change-pipeline`）自动运行时同样适用："能查"不是"能替用户拍板"的许可。查事实可以派子 agent 去查，且**不阻塞本轮**：查询进行中等于前置未定，只有依赖它的问题等结果，前沿的其余问题照问。
4. **前沿为空才算问完**：每个分支都走到、没有默默假设的东西；用户的答复解锁依赖它们的子决策，前沿随之外推。
...
3. **按前沿逐轮追问**：第一轮问当前前沿的全部问题，按关键度排序编号，每题固定格式：

   ```text
   ❓ **Q1** - **<问题标题>**: <正文，可多段，可列多个选项>

   ➡️ <推荐答案 + 一句理由>
   ```

4. **重算前沿**：用户答完一轮后，把新解锁的子决策纳入前沿再问下一轮。事实查询按成本分级：一条命令、一个文件能查的当场查；跨多文件的查询一轮合并派**一个**子 agent，不是一题一个。
...
- **过程中**：逐轮的前沿问题清单——每题编号、固定格式、带推荐答案；一轮之内的问题彼此不依赖。
...
改编自 [`mattpocock/skills`](https://github.com/mattpocock/skills) 的 `grill-me` 与 `grill-with-docs`（中文参考 [`vinvcn/mattpocock-skills-zh-CN`](https://github.com/vinvcn/mattpocock-skills-zh-CN)），按本仓库 skill 规范本地化，并接入 `stage-change-pipeline` 工作体系；按前沿逐轮提问的访谈协议与固定问题格式同步自其 v1.2.x 的 `grilling`。上游的文档持久化变体在本仓库合并为本 skill 的 docs 模式（沉淀落点为 `openspec/glossary.md` 与 `docs/adr/`）。
