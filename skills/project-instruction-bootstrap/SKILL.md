...
- 检测现有 `CLAUDE.md`/`AGENTS.md`：是否存在、是否已是**本 skill 的**生成物（有 `instructions/agents/` 源 + 本 skill 的 do-not-edit 头）、已覆盖哪些段落。`eng-init` 写的 `AGENTS.md`（头部 "Generated by `eng-init`"，或 `constraints.yaml` 有 `generated_sections.agents_md`）**不算**本 skill 的生成物——按手写处理，走增量兼容模式。
...
`eng-init` 是控制面引擎，拥有 `AGENTS.md` 里控制面段落的**内容**（命令、验证矩阵、边界、enforcement index……）；本 skill 拥有生成型项目根指令文件的**写入机制**。同一项目两者并存，不二选一：

- **项目已用本 skill 的生成模式**（有 `instructions/agents/` 源、`AGENTS.md` 带本 skill 的 do-not-edit 头）→ `eng-init` 把它的段落写进 `instructions/agents/shared.md`，再重跑本 skill 重生成；它不直接改生成物。
- **否则** → `eng-init` 直接写 `AGENTS.md`。本 skill 之后只走增量兼容模式：绝不覆盖 `eng-init` 在 `constraints.yaml` `generated_sections.agents_md` 登记的段落；Claude Code 侧的 `CLAUDE.md` 用一行 `@AGENTS.md` 导入再接 Claude 增量段，不把共享内容复制两份——Claude Code 只读 `CLAUDE.md`，不读 `AGENTS.md`，没有这一行整个控制面对它不可见。
- 可向用户提议把 `eng-init` 的段落迁入生成模式（同增量模式的迁移提议），不强制。
