...
本 skill 要你贴命令、输出与捕获物。**先脱敏再贴**：secret 一律以 `<REDACTED>` 占位；回路对着环境变量构建，凭证留在环境里而不进你展示的内容；捕获物（HAR、日志转储）带 auth header，只引用承载信号的那几行。脱敏后的输出不足以诊断时，说明并向用户要。
...
**建不出回路**：停下，列出已尝试的构建方式，向用户要（a）环境访问，（b）脱敏后的捕获物——HAR、日志转储、core dump、带时间戳录屏，或（c）临时生产插桩许可。**没有回路，不进假设。**

**完成判据（硬闸门）**：说出一条**已经跑过至少一次**的命令（贴出调用与输出，脱敏后），同时满足四项——**能红**（驱动真实 bug 路径、断言用户的确切症状）、**确定**、**快**（秒级）、**代理可跑**（人只经 HITL 脚本参与）。发现自己在读代码编理论而这条命令还不存在——**停：直接跳到假设正是本 skill 要防的失败。没有能红的命令，就没有 Phase 2。**
...
改编自 [`mattpocock/skills`](https://github.com/mattpocock/skills) v1.1.0 的 `diagnosing-bugs`，脱敏节同步自其 v1.2.x。持久层本地化：上游的 `CONTEXT.md`/`docs/adr/` 改为本仓库的 `openspec/glossary.md` 与 `docs/adr/`。上游"把假设清单拿给用户看"的 HITL 检查点在管道（AFK）场景降级为证据记录；HITL 回路脚本保留用于独立场景。多代理模式吸收自环境侧 UltraThink Debug Orchestrator（原 `/debug` 命令，已由本 skill 取代）：保留其多视角扇出与用户确认点，丢弃其"假设先于回路"的顺序与固定四角色输出格式。
