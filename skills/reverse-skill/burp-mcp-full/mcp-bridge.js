...
async function fetchTools() {
  return new Promise((resolve, reject) => {
    http
      .get(`http://${BURP_HOST}:${BURP_PORT}/tools`, (res) => {
        let data = "";
    ...
    case "tools/list":
      if (!TOOLS)
        return {
          jsonrpc: "2.0",
          id,
          error: { code: -1, message: "Tools not loaded. Is Burp running?" }
        };
    ...
    case "tools/call": {
      ...
      return callTool(toolName, toolArgs)
        ...
        .catch((err) => ({
          jsonrpc: "2.0",
          id,
          error: { code: -1, message: err.message || "Tool call failed" }
        }));
    }
...
async function main() {
  ...
  try {
    ...
    process.stderr.write(
      `[burp-mcp-bridge] WARNING: Cannot connect to Burp at ${BURP_HOST}:${BURP_PORT}. Start Burp first.\n`
    );
    TOOLS = null;
  }
