...
**Shared vocabulary, per-object instantiation.** This file is the entropy suite's canonical abstract vocabulary, consumed by `repo-entropy-audit` (whole-repo scans), `review`'s consistency mode (per-change reviews, via the diff-granularity dimension mapping in its `consistency-checklist.md`), and `control-plane-auditor` (which additionally applies its own seven-layer model to the control *system* itself). The axes are shared; the detection methods are not — each consumer instantiates them for its own object and granularity. Do not force one checklist across objects: a unified abstract language with divergent concrete probes is the intended design.
