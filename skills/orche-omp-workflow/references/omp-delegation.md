...
```bash
codeagent-wrapper --backend omp --model <MODEL_PIN> --output <OUT_JSON> - <WORKDIR> <<'OMPTASK'
...
| Role | Pin | Notes |
| --- | --- | --- |
| `implementer` | `sub-claude/claude-opus-4-8` | Verified working. |
| `verifier` | `sub-claude/claude-opus-4-8` | Verified working. |
| `reviewer` | `openai-codex/terra:max` | Verified working, including the `:max` suffix. |

Rules:

- The pins above are the workflow's defaults, not omp's. Omitting `--model` falls back to
  omp's own `default` role from `~/.omp/agent/config.yml`, which is a different model than
  this workflow assumes — always pass `--model`.
- Reasoning effort belongs **in the model string** (`provider/model:effort`, one of
  `off|minimal|low|medium|high|xhigh|max`). `--reasoning-effort` is accepted by the wrapper
  and then silently dropped for the omp backend — never rely on it.
- **Known limitation**: the `:effort` suffix resolves only for models the local omp model
  cache lists authoritatively. On a provider whose discovery cache is empty, appending an
  effort (`sub-claude/claude-opus-4-8:max`) fails with `Model "…" not found` while the bare
  form resolves. Verify a new pin with a one-line throwaway task before using it in a run;
  when the suffix fails, use the bare pin and record in the evidence bundle that effort fell
  back to the model default.
- Verify pins against `omp models` when a run starts on a new machine. A pin that does not
  resolve fails the task at exit code 1 with `Model "…" not found` — that is a tooling
  failure, not a review round.
...
4. **omp delegation boundary** (below) — replaces `SKILL.md`'s native subagent boundary.
5. **The phase brief** — exactly as specified by `phase-flow.md` / `phase-4-cross-review.md`.
...
- `backend` and `model` are per-task headers — set them on every block; the global `--backend`
  default is `codex`, which is not what this workflow wants.
