...
**Execution substrate**: every `implementer`/`reviewer`/`verifier`/`explorer` task below runs as an omp session via `codeagent-wrapper --backend omp` — **except the Phase 7 final review, which runs as a native Claude Code / Codex subagent**. Invocation syntax, model pins, brief assembly (role contract + operating-guide pointer + boundary), parallel mode, and failure classification are all defined in `omp-delegation.md`; read it before the first delegation. Where a step below says "delegate an `implementer` task", it means the omp form unless the step says native.
...
1. Scan the repo for risk-bearing structure: primary language/build system, public entrypoints (CLI, API, services), data schemas/formats/serializers, external integrations and credentials, persisted/shared state, and shared helper roots — plus the command entry points (package scripts, Makefile/justfile targets, CI workflow steps) for setup/check/lint/typecheck/test/build. On anything larger than a single-package repo, delegate this sweep to one read-only omp `explorer` task (`omp-delegation.md`: explorer pin, evidence-only brief line, read-only boundary) and ask it for exactly those categories with file paths; a repo small enough to read directly does not need a delegation. The explorer returns the map — steps 2-4 (profile choice, field authoring, budget) stay with the orchestrator.
...
1. Create a short triage from issue, repo context, expected change surface, and the active project profile (`openspec/project-profile.md`).
   - Impact mapping: when the expected change surface is not already obvious from the issue and the profile — an unfamiliar subsystem, a shared helper, a suspected multi-surface blast radius, or anything heading for `high`/`broad-expanded` — delegate one read-only omp `explorer` task first (`omp-delegation.md`: explorer pin, evidence-only brief line, read-only boundary). Give it the issue text and ask for the concrete surfaces: producers, validators, storage/query, public entrypoints, downstream consumers, failure/rollback paths, and the existing tests covering each, with file paths and line-level evidence. Its map is what makes steps 8-9 (`Invariant Matrix`, boundary-surface checklist) real rather than guessed. A one-file, well-understood change skips this.
   - The explorer never assigns the fixture level, repair intensity, or risk packs — steps 2-4 are orchestrator decisions made from its evidence. A report that arrives carrying those conclusions has over-reached; keep the evidence, drop the verdict.
...
10. Run one focused fixture review as a read-only omp `reviewer` task (`omp-delegation.md`; single task, no worktree); it checks only fixture completeness. For high or broad-expanded fixtures, the review must specifically validate the `Invariant Matrix`; if it is absent or too vague, the fixture review is `revise`.
...
## Phase 1: omp Implementer Implementation

1. Ensure the delegation substrate is ready: `codeagent-wrapper --backend omp` resolves, the `implementer` model pin resolves in `omp models`, and the `implementer` agent definition (contract bullets + `references/operating-guide.md`) is installed in the host project. A missing pin or missing agent definition is an escalation, not a degraded run.
...
4. The implementer brief must include:
   - The role header, the `implementer` contract bullets, and the operating-guide pointer (`omp-delegation.md` → Injecting the Role Contract). Without them the omp session runs as a generic coding assistant.
   - The omp delegation boundary from `omp-delegation.md`.
...
6. Use timeouts by complexity: simple 30 min, medium 1 h, complex 2 h. **Never kill a running omp task**; `[codeagent-progress]` frames mean it is alive. While it runs, wait silently with a long tool timeout; do not poll every few seconds or stream intermediate logs unless diagnosing a stuck/failing task.
7. Classify a non-zero exit or missing report per `omp-delegation.md` → Timeouts and Failure Handling: a tooling failure is diagnosed once and re-run; a task failure means refining the brief and retrying, up to 2 attempts.
...
- A repository policy that requires a fixed number of evidence comments never raises the seat count; post a consolidated evidence bundle instead.
- The caps are enforced at ledger time, not at spawn time: `review_gate.py open --fixture <level>` arms the per-level round-1 cap (4 when omitted), and `record-round --lenses <seat list>` refuses an off-vocabulary or duplicated lens with nothing recorded, while a round that ran with more seats than its cap is recorded with a `VIOLATION` ledger line and exit 2 — its tokens are already spent, so it is recorded, not hidden.

When Phase 4 starts, open the gate CLI with the issue binding: `review_gate.py open --pr <N> --issue <M> --fixture <level>` — the `--issue` flag arms the cross-PR ceiling memory (`gates.md`), which is what makes a successor PR for a ceiling-hit issue escalate instead of restarting the argument from zero.
...
Include the PR description's `偏离记录` section in every reviewer brief: deviations are where the implementer made choices the plan did not cover, so review attention goes there first. Use `phase-4-cross-review.md` to build the parallel omp reviewer briefs. Spawn the full reviewer set as one `codeagent-wrapper --parallel` batch, one `---TASK---` block per reviewer with `backend: omp` and the reviewer model pin (`omp-delegation.md` → Parallel Delegation). Reviewer tasks are read-only, share the PR worktree, and return their complete reports as their final message; the orchestrator takes each report from the `--output` JSON and persists it to `<REVIEW_DIR>/<report file>` (default `<REVIEW_DIR>` = `.workplans/<issue-or-pr>/review/`), alongside the task's `session_id` and `log_path`. Do not post PR comments in this phase.

Review rounds:

- Round 1 uses the seat plan above.
- After a Phase 6 fix pass, rerun cross-review before Phase 7 on the current head (classified exceptions rerun only their own gates: Phase 8 `ci-only` repairs and Phase 7 `local-repair` fixes). Post-fix rounds default to the **pinned risk-pack core only** (2 seats; hard cap 3): the lenses selected by the fixture's risk packs are pinned, present in every round, and carry the fix-regression recall. Free slots — complementary lenses from reviewer packs not yet used on this PR (`risk-adaptive-cross-review` `reviewer-packages.md`) — rotate back in only when the previous round had a critical/major finding or a failure-class repeat (ledger `repeats prior class: yes`); rotation fills the one free seat within the cap and never rotates out a pinned lens. This is a cost governor: comprehensive rounds are the most expensive unit in the workflow, so lens breadth is bought only when the ledger signals it. Record each round's seat list in the evidence bundle and in `record-round --lenses`; the policy is adjudicated by the review-loop log's lens attribution (Phase 8), keep/cut via the existing ADR mechanism.
...
- If the parallel batch fails without producing reports — wrapper non-zero exit, unresolvable model pin, empty task result — diagnose once per `omp-delegation.md`, then re-run the same reviewer set. Do not count a failed no-report invocation as a comprehensive review round, and append no ledger line for it.
- A reviewer report from a session that could not read its operating guide, or that was briefed without the `reviewer` contract bullets, is degraded: discard it and re-delegate that lens. A round with a discarded lens is not complete until the replacement returns.
...
- If verification tooling fails without producing verdicts, rerun the batch once; do not silently promote all candidates to blocking, and do not silently drop them. The same tooling-vs-task failure classification as Phase 1 applies (`omp-delegation.md`).
- A verifier task must not be given the reviewer briefs or reviewer session ids from the same round — it adjudicates the candidate texts only. Reusing an omp session (`resume <session_id>`) that produced candidates for the verification of those candidates violates the producer/judge separation exactly as a shared subagent would.
...
## Phase 6: omp Implementer Fix Pass

Fix passes and diagnosis tasks are omp `implementer` delegations, assembled and launched exactly as in Phase 1 (role contract + operating-guide pointer + omp delegation boundary + brief; `omp-delegation.md`). In every template below, `<Required subagent boundary>` means the **omp delegation boundary**, not the native one.
...
Diagnosis brief template — delegated to an omp `implementer` task as a report-only task (no fix). Vocabulary is canonical in the `diagnosing-bugs` skill; the orchestrator inlines this distilled brief because delegated omp sessions run with `--no-skills` and cannot load it:
...
The orchestrator may do the audit read-only, or delegate one read-only omp `reviewer` task. The audit must not edit files. It must answer:
...
2. Spawn a clean-context **native** `reviewer` subagent (Claude Code Task subagent or Codex subagent) for read-only final review, carrying the native subagent boundary from `SKILL.md`. It must not edit files, invoke this workflow, use skills, or spawn further subagents. **This phase must not be delegated to omp** — every earlier agent-executed phase ran on one engine, and the last gate before merge is the workflow's only cross-harness independence (`omp-delegation.md` → Why Phase 7 stays native). Running Phase 7 on omp is a gate failure: discard the result and re-run it natively.
...
- **Validation before append**: compose the line into a local file and run `scripts/evidence_check.py --loop-log-entry <file>` first — required keys, outcome vocabulary, the fixture token, the lens vocabulary of `catches`, and the seat caps of `round_lenses` (round 1 by fixture level, 3 afterwards; list length equals `rounds`) are machine-checked. `fixture` is the effective tier as **one canonical token** (`high`/`broad-expanded` subsume `expanded` when repair intensity escalated): composites like `expanded/high` and ad-hoc labels like `standard` fragment the per-level keep/cut sample and are rejected.
...
- `round_lenses`: the seat list actually used per round (index 0 = round 1); a seat is one lens id or an `a+b` pair, exactly as passed to `record-round --lenses` — the instrumentation for the pinned-core + rotating-free-slots trial and for the seat caps.
- `catches`: one entry per `gate_net_catch` finding — the round it surfaced, the lens that produced it, its failure class and severity. `lens` is exactly one lens id: a seat lens (`correctness`, `integration`, `security-perf`, `test-evidence`, `spec-compliance`, `invariant-state` — the checklist that produced the finding, never the `a+b` seat) or a phase lens (`fixture-review`, `final-review`, `gap-sweep`, `invariant-audit`). This attributes later-round catches to the pinned core (fix-regression recall), a rotated-in lens (blind-spot recall), or a non-round phase.
...
**Consume the log, don't just append to it**: after appending, run `scripts/loop_log_audit.py --log docs/review-loop-log.jsonl`. Exit 2 means a decision is now owed — a fixture level has reached the keep/cut sample with zero net catch, or the lens-rotation trial has reached its attribution sample. Record the ADR in `docs/adr/` (or a one-line recorded deferral with reason) before starting the next issue. The `NOTE per-lens yield` line (seated rounds, catches, P0/P1 per seat lens across merged PRs) is informational: it is the cost signal for the next seat-plan revision in `risk-adaptive-cross-review`, and owes no ADR.
