...
Use this reference for any parallel code-writing delegation in this workflow, including Phase 1 implementation and Phase 6 fix passes. Every worker here is an omp `implementer` task launched through `codeagent-wrapper` (`omp-delegation.md`).

This reference does not apply to read-only fixture review, cross-review, invariant audit, or final review tasks. Read-only tasks share the PR worktree and run as a plain parallel batch with no worktrees.

## Non-Negotiable Rules

- Parallel code-writing workers must use separate git worktrees under `.worktrees/`, created by the orchestrator and passed to each worker as its `workdir:`. Do not use `codeagent-wrapper --worktree`: it auto-generates a `do/{task_id}` branch outside this manifest, which defeats the disjoint-write-set audit and the cleanup checklist below.
...
Then launch the workers as one parallel batch, each pinned to its own worktree:

```bash
codeagent-wrapper --parallel --output .workplans/<N>/impl-batch.json <<'OMPBATCH'
---TASK---
id: <worker-id>
backend: omp
...
Scope notes: the hook guards the **orchestrator's own** file-edit tools (`Edit|Write|MultiEdit|NotebookEdit` on Claude Code, `apply_patch` on Codex). It does not reach inside a delegated omp session — omp workers edit through their own tools in a separate process, so their confinement rests entirely on the `workdir:` they were given plus the allowed-write-set audit at integration time. Patch integration via `git apply` in the shell is unaffected, and shell-level `mv`/`cp`/redirects are not covered. Without the hook installed, this entire section is a no-op and the discipline stays orchestrator-enforced.

## Worker Prompt Boundary

Every parallel code-writing worker brief is assembled as in `omp-delegation.md` (role header + `implementer` contract bullets + operating-guide pointer + omp delegation boundary + brief), plus this parallel-worktree boundary:
