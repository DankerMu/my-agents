...
Run one short read-only omp `reviewer` task for every OpenSpec fixture before implementation (`omp-delegation.md`: role header + `reviewer` contract bullets + operating-guide pointer, then the block below). The review checks only whether the selected risk packs and evidence are sufficient.
...
- Do not spawn omp task agents, call codeagent-wrapper, or ask any other AI/code agent to review, fix, implement, verify, or plan.
- Do not ask another agent to implement, fix, review, or plan.
- Use ordinary shell/read-only tools only. Do not edit files, commit, push, or change any state.
