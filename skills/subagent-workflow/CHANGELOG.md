...
- **Phase 4 reviewer set becomes a capped seat plan; the six-reviewer escalation is deleted** (consumes `risk-adaptive-cross-review` 0.5.0). Round 1: `none` 0, or 1 seat when the Phase 2 audit flags risk; `compact` 1-2 seats (`correctness+test-evidence`, plus `integration`/`security-perf` when a risk pack names it); `expanded` 2-3, cap 3 (`correctness`; `test-evidence+spec-compliance`; one risk-pack lens); `high`/`broad-expanded` exactly 4 (`correctness`; `invariant-state`; `test-evidence+spec-compliance`; `security-perf+integration`). Post-fix rounds: pinned core 2 seats, cap 3. Motivation from four repos' loop logs (NWM 523 lines, xagent 138, yd-viewer 22, open-workbuddy-web 14): the "escalate to 6 when the PR touches DB state / retry / publish / schema / security / prod config / shared state machines" list was the definition of `high`, so 6 was the de facto default (NWM 45/58 high PRs, xagent 28/29, owb-web 11/11, 10-12 reviewer-runs per high PR); in six-seat round 1 at most one lens produced a P0/P1 in 29/48 (NWM) and 27/31 (xagent) rounds; `integration` had the lowest catch-per-run in all four repos; `spec-compliance` catches were mostly P2 and near-zero outside spec-heavy NWM; `invariant-state` was low-volume with the highest P0/P1 share, so it keeps its seat. Expanded rounds were also running 6 against a written cap of 4 (xagent 17/34, yd-viewer 8/16) and post-fix rounds averaged 3.3-4.0 seats against a written "2-3" — prose caps drift, hence the next item.
- **Seat caps are mechanical (`scripts/review_gate.py`)**: `open` takes `--fixture <level>` (arms the round-1 cap: none 1 — the single seat a Phase 2 risk flag buys — compact 2, expanded 3, high/broad-expanded 4; 4 when omitted); `record-round` requires `--lenses a,b+c` (the seats that actually ran; lens ids canonical in `risk-adaptive-cross-review` — a `review-` task-id prefix and the `security-performance` long form, the spellings the four live logs actually carry, are canonicalised so nothing already running is refused). An off-vocabulary or duplicated lens is orchestrator paperwork — refused, nothing recorded (same shape as the `--clean` conflict). A round with more seats than its cap already spent its tokens — recorded, tagged `seatCapExceeded`, `VIOLATION` ledger line, exit 2 (same shape as recorded-while-locked). Every ledger line now carries `seats: <list>`. Enforcement is at ledger time, not spawn time: the `review-gate` hook still reads only `locked`.
- **Loop-log lens vocabulary is enforced (`scripts/evidence_check.py --loop-log-entry`)**: `catches[i].lens` must be exactly one lens id — a seat lens or a phase lens (`fixture-review`, `final-review`, `gap-sweep`, `invariant-audit`), never an `a+b` pair; `round_lenses`, when present, is checked for canonical seats, no lens in two seats of one round, the round-1 cap by fixture, the later-round cap of 3, and a length equal to `rounds`. Motivation: NWM's log carried 100+ free-text lens labels, which is the attribution key `loop_log_audit.py` derives the pinned core from.
- **`scripts/loop_log_audit.py` attribution survives paired seats**: the round-1 core set splits `a+b` seats into both lens ids (a `spec-compliance` catch is core when round 1 seated `test-evidence+spec-compliance`), and phase-lens catches are reported in a new `phase=` bucket instead of being miscounted as rotated-in (NWM: 51 later-round phase-lens catches, mostly `final-review`, were inflating `rotated`).
- `phase-4-cross-review.md`: the stale "follow-up rounds run the same reviewer count" line (contradicting phase-flow's pinned-core rule since 0.2x) is gone; the roles table is a seat table; the brief's finding shape adds `Lens` so paired-seat findings stay attributable; the "repository policy requires N evidence comments" clause no longer references a six-reviewer escalation.
