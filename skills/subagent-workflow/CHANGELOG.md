...
- **`scripts/loop_log_audit.py` attribution survives paired seats**: the round-1 core set splits `a+b` seats into both lens ids (a `spec-compliance` catch is core when round 1 seated `test-evidence+spec-compliance`), and phase-lens catches are reported in a new `phase=` bucket instead of being miscounted as rotated-in (NWM: 52 later-round `final-review` catches were inflating `rotated`).
