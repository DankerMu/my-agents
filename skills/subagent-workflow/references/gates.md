...
```text
Round <N> | <sha> | clean|not-clean | verified findings: <n> | highest severity: <sev> | failure classes: <list> | repeats prior class: no|yes (<class>, also round <M>) | seats: <seat list> | gate: none|three-round|working-day|same-invariant
```
...
- `verified findings: <n>` counts only candidates verdicted CONFIRMED/PLAUSIBLE **with disposition FIX_NOW** in the Phase 4.5 pass. DEFER and DISCARD candidates never enter the count or trigger gates; they stay in the `verify-<CLASS_ID>.md` audit trail, and every DEFER joins the existing round-close routing (issue-scribe URL or recorded reason) — except in a P2-only round, where the rule below makes them notes instead.
- A round whose only verified findings are minor (P2) (coverage findings excluded — they always fix, and a round carrying one is not clean) is recorded **clean** with those P2s noted, not deferred: they stay in the round's reviewer report and verifier verdict table inside the evidence bundle, owe no issue-scribe URL and no recorded reason, and never reach the loop-log `residual_deferred` or the ledger line. No gate fires on a round the P2-note default exits anyway.
...
**Preferred mechanics — the packaged gate CLI** (`<skill>/scripts/review_gate.py`): run `open --pr <N> --issue <M> --fixture <level>` once when Phase 4 starts (`--issue` enables the cross-PR ceiling memory below; `--fixture` arms the round-1 seat cap), then `record-round --sha <sha> --lenses <seat list> --clean|--not-clean [--verified <n> --highest <sev> --classes <list>]` after each round. The seat list is the reviewers that actually ran (`a` or `a+b` per seat, lens ids from `risk-adaptive-cross-review`): a `review-` prefix and the `security-performance` long form are canonicalised; any other off-vocabulary or duplicated lens is refused with nothing recorded, and a round with more seats than its cap (round 1: none 1 / compact 2 / expanded 3 / high and broad-expanded 4, or 4 without `--fixture`; later rounds 3) is recorded, tagged `seatCapExceeded`, and gets a `VIOLATION` ledger line with exit 2. It maintains `.review-gate.json` at the project root (precomputed `locked`/`lockReason`), auto-detects failure-class repeats against all prior rounds, appends the ledger line to `<REVIEW_DIR>/round-ledger.log`, exits 2 the moment a gate locks — immediate feedback, not a pre-merge discovery — and refuses any round beyond the 5-round ceiling. `record-round --clean` refuses any explicit finding argument (`--verified`, `--highest`, `--classes`) instead of silently discarding it — a clean round is `0 / none / []` by definition, and P2s noted by a P2-only clean round live in that round's review/verification records, not in these fields. A misrecorded round SHA is corrected with `correct-round --round <N> --sha <sha> --reason <text>`: the round keeps old SHA, new SHA, and reason under `shaCorrections`, the effective SHA updates for `status` and later evidence, and one `CORRECTION | sha <old> -> <new> | reason: <text> | round <N>` line is appended — the original round line is never rewritten, and the correction changes no counter, metric, repeat, lock, or budget (the SHA is audit-only; no gate math reads it). Register a persisted retro with `record-retro --path <file> --shape <shape>` (it mechanically enforces `converging` and `depth` eligibility and arms the post-gate budget), manual triggers with `lock --reason <trigger>`, and run `close --outcome merged|superseded-by-split|abandoned|descoped` after merge or abandonment. With the optional `review-gate` hook installed, a locked state mechanically denies implementer/reviewer spawns until `record-retro` runs; independent of the hook, `evidence_check.py` fails on a locked state, so a skipped gate transition is caught at the next spawn/post point instead of surviving out of band. Gitignore `.review-gate.json` (per-PR ephemeral state); **commit `.review-gate-issues.json`** (the cross-PR memory).
