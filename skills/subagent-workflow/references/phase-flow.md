...
- A cross-review round is clean when it leaves no critical/major (P1+) finding to fix and no coverage gap — P1+ remains return to Phase 5-6, and any genuine deferral taken out of a fix pass is routed at round close. A remainder of only minor (P2) findings does not make the round dirty; those P2s are noted, not routed (rule below). What a verified finding buys is decided by severity — the unit being rationed is the post-fix comprehensive re-review round, the most expensive unit in the workflow, and it is bought only for P1+ (`finding-contract.md` Severity Crosswalk: P0=critical, P1=major, P2=minor):
  ...
  - **When verification leaves only minor (P2) findings — no critical/major, no coverage gap — the round is recorded clean and the P2s are noted, not deferred.** They stay where the round already wrote them: the reviewer report (`<REVIEW_DIR>/<lens>.md`) and the verifier verdict table (`<REVIEW_DIR>/verify-<CLASS_ID>.md`), which the evidence bundle carries. Nothing further is owed — no `issue-scribe` filing, no recorded one-line reason, no `residual_deferred` entry (they are not deferrals) — and the Phase 8 deferral-routing gate does not fire on them. Call this out in the Phase 8 work summary as a one-line "P2 notes: <n> (see `<REVIEW_DIR>`)" so it is visible, not silent. Noted P2s still count as catches for lens attribution. Then proceed to Phase 7. Never spawn a fix pass plus re-review round for P2-only remains. Filing an issue for a noted P2 is always allowed as a judgement call; it is simply not required.
  - **Coverage carve-out (non-negotiable)**: `test-evidence`/coverage findings for behavior this PR introduces are never downgraded to a note and never deferred regardless of assigned severity — they buy the fix pass like a P1+, and the existing `wontfix` ban stands. A round still carrying one is not clean.
  - Deferral routing itself is unchanged and hard for genuine deferrals — a verified finding dropped out of a P1+ fix pass on Downgrading rationale, or any verifier `DEFER` in a round that was not P2-only. (Noted P2s from a P2-only clean round are not deferrals and are exempt.) When the `issue-scribe` agent is installed, "deferred with issue basis" means delegating the deferred finding to `issue-scribe` (it verifies, dedups, and files the tracked issue) and recording the returned issue URL in the evidence bundle; a deferral with neither an issue URL nor a recorded reason is not a valid deferral, and the Phase 8 gate blocks on it.
...
- Phase 5 exists only when P1+ triggered it — a P2-only verification result exits at Phase 4 clean with its P2s noted in the round's review/verification records and never reaches fix synthesis. When Phase 5 does run, verified P2s are on the checklist by default (ride-along); dropping one requires the recorded Downgrading rationale from the Phase 4 round rule.
...
- **Deferral routing**: every explicitly deferred finding and every 剩余风险/已知限制 entry in the work summary carries either a tracked issue URL (delegated to `issue-scribe` when installed) or a recorded one-line reason why no issue is filed. An unrouted deferral is a gate failure, not a silent drop. P2s noted by a P2-only clean round (Phase 4 round rule) are not deferrals: they owe no issue and no reason line, and this gate does not fire on them — report their count and location in the work summary instead.
...
`residual_deferred` counts genuine routed deferrals only; P2s noted by a P2-only clean round are not deferrals and stay out of it (they appear in `catches` like any other verified finding).
