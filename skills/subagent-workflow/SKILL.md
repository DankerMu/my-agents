...
- **Reviewers produce candidates; verify before fix**: Phase 4 reviewers emit candidates only. Dedup, group by failure class, and run one independent `verifier` per class batch (≤5 candidates, one verdict each: CONFIRMED/PLAUSIBLE/REFUTED) — never a reviewer that produced a candidate in the batch, never the orchestrator self-judging. Only CONFIRMED plus risk-weighted PLAUSIBLE enter Phase 5. Fixture-level bias and mechanics in `references/phase-flow.md` Phase 4.5. **Severity rations the re-review round**: P1+ (and coverage gaps, always) buy the fix pass; when one runs, verified P2s ride along on the same checklist by default; a P2-only verification result records the round clean with those P2s noted in the round's own review/verification records — not deferred, not routed — and goes straight to Phase 7; a comprehensive round is never bought for P2s alone (severity crosswalk canonical in `risk-adaptive-cross-review` `finding-contract.md`).
- **Review vocabulary is canonical in `risk-adaptive-cross-review`**: reviewer packs, risk-triage levels, the finding contract, and failure-class synthesis are consumed, never restated or forked here. Reviewer seats per round are capped by fixture level (round 1: compact 2, expanded 3, high/broad-expanded 4; later rounds 3) and every round's seat list is recorded with `review_gate.py record-round --lenses`, which refuses off-vocabulary seats and ledgers over-cap rounds as violations.
- **Ordinary-loop gates are governed by `references/gates.md`** (round ledger, gate table, retro template, failure shapes, post-gate budgets). Two non-negotiables override any reference detail: (1) once the third comprehensive cross-review round returns not clean, no further ordinary action — fix, review round, Phase 7, CI wait, or merge — runs before the Review Failure Retro is persisted, and what runs next must be its chosen corrective action; a declining finding count is not an exemption. (2) The round counter (the per-round ledger) never resets within a PR; a fresh counter is legitimate only for a child PR created by a gate-selected PR split. The ceiling additionally has **cross-PR memory per issue** (`open --issue`, `.review-gate-issues.json` survives `close`): a successor PR for an issue that already hit the ceiling refuses `depth`/`noise` retros without a recorded user decision — the split/descope/stop call reaches the human at the start of the successor PR, not after its budget burns.
...
- **Cross-run loop accountability**: after each merge **and each terminal gate outcome** (round-ceiling split, abandonment, descope), append one line to `docs/review-loop-log.jsonl` — every merged PR gets its own line, split/batch children included; terminal lines carry `outcome` and feed the upstream sizing-retro. Validate the pending line with `evidence_check.py --loop-log-entry` (canonical fixture token, outcome vocabulary), then run `loop_log_audit.py`: a DECIDABLE result obligates a keep/cut ADR in `docs/adr/` (or a recorded deferral) before the next issue. Keep/cut decisions stay recorded human calls, default keep. Mechanics in Phase 8.
...
Phase 8: evidence + Chinese work summary + CI + pre-merge evidence hard-gate (incl. branch-tip integrity) + merge gate or pre-authorized auto-merge + openspec archive + append cross-run review-loop accountability log (merged and terminal lines) + loop_log_audit keep/cut check
```
