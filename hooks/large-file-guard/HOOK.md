...
```bash
npx my-agents install hook large-file-guard            # 双平台
...
- 只守 Claude Code `Bash` / Codex `exec_command` 工具发起的提交；终端里人工 `git commit` 不经过本 hook（要全覆盖可另配 git 原生 pre-commit，v1 不代管）。
