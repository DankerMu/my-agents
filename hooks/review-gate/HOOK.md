...
3. `locked: true` 时，子代理派发调用（Claude Code：`Task`，按 `subagent_type` 匹配；Codex：`collaboration.spawn_agent` / `followup_task`，按 payload 中的 agent 名字段匹配）命中 `blockedSubagents`（默认 `implementer`、`reviewer`）即被拒绝；`verifier` 及其他辅助子代理（explorer、monitor、issue-scribe）不受影响——裁决刚结束的轮次仍需要它们。
...
```bash
npx my-agents install hook review-gate            # 双平台
