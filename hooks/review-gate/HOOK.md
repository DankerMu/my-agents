...
3. `locked: true` 时，所有 `Task` 派发中 `subagent_type` 命中 `blockedSubagents`（默认 `implementer`、`reviewer`）的调用被拒绝；`verifier` 及其他辅助子代理（explorer、monitor、issue-scribe）不受影响——裁决刚结束的轮次仍需要它们。
...
安装动作 = 拷贝脚本到 `<project>/.claude/hooks/review-gate/` + 把配置片段幂等 merge 进 `<project>/.claude/settings.json`。卸载按 deep-equal 摘除本包写入的条目。
...
- 只覆盖 Claude Code（matcher `Task` + `subagent_type` 字段）。Codex 侧没有稳定的子代理派发工具名可 match，本包不出 codex fragment；Codex 编排下的机械层是 CLI 本身——非法转移在 `record-round` / `record-retro` 时被当场拒绝（exit 2），带违规标记写入 ledger，pre-merge 硬门再做确定性兜底。
- 拦截依赖 `subagent_type` 命名与 `blockedSubagents` 匹配；用自定义 agent 名跑 implementer 职能可绕过——那属于故意违规，由 ledger 违规行 + accountability log 兜底。
