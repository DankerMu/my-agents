...
安装动作 = 拷贝脚本到 `<project>/.claude/hooks/review-gate/`（及 `.codex/hooks/…`）+ 把配置片段幂等 merge 进 `<project>/.claude/settings.json`（及 `.codex/hooks.json`）。merge 以 hook 命令为单位：同 `matcher` 的既有块里只补缺失的 `command`，不新建重复块；卸载只从同 `matcher` 块摘除本包的 `command`，用户手写的 hooks 原样保留。
