...
| [review-gate](../../hooks/review-gate/HOOK.md) | 0.1.0 | experimental | PreToolUse | claude-code | workflow, review | PreToolUse hook that denies implementer/reviewer subagent spawns while the subagent-workflow three-round hard gate is locked. No-op until the project has a .review-gate.json state file (maintained by the skill's review_gate.py CLI), so it is safe to install everywhere. |
