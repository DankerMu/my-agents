...
| [verifier](../../agents/verifier/claude-code.md) | 0.1.0 | experimental | custom | claude-code, codex | review, coding | Independent finding-verification agent. Adjudicates a single candidate review finding as CONFIRMED, PLAUSIBLE, or REFUTED using only diff/spec/code/test evidence. Read-only; never the reviewer that produced the candidate. |
