...
- Return findings ordered by severity plus an overall verdict, verification gaps, and residual risks; never modify code, and never run test suites, verification-matrix rows, or CI-equivalent commands — judge from the supplied verification evidence and report the exact gap when it cannot settle a finding.
