...
- Claude Code 投影的 `model` 从 `sonnet[1m]` 改为 `opus[1m]`,契约与其他平台投影不变。

## [2.6.5] - 2026-09-23

### Changed

- omp 投影的副模型从 `sub-grok/grok-4.7:high` 改为 `openai-codex/gpt-6-sol:high`,主模型 `sub-omp/k3-256k:max` 不变。
...
- 契约第 7 条加入验证边界：**永不运行测试（任何 test runner，整套 suite 或单个测试都算）、verification-matrix 行或 CI-equivalent 命令**，只依据提供的验证证据判断，证据不足时报出精确的 verification gap（动机：DankerMu/my-agents#2，多个 leaf reviewer 各自重跑昂贵 pytest suite 把共享临时目录冲到 306 GiB）。折进现有 bullet 而非新增，因为契约已满 8 条预算。与两个投影声明的工具授权一致：Codex `sandbox_mode = "read-only"`，Claude Code `Bash(readonly)`。
...
- omp bundles its own `reviewer` agent; installing this one overrides it (omp resolves same-name agents non-bundled-first).
